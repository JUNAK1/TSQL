...
la cual consolida la información en la tabla fisica tbConsolidadoFranjasEstadoRealTime, que es utilizada para realizar la vista vwRepRTAuxMeli
<Ejemplo>
...
SET NOCOUNT ON;
    ...
    BEGIN
        ...
        /*======================================== Truncate a la tabla fisica para insercion de nuevos datos   ======================================*/
            ...
            TRUNCATE TABLE tbConsolidadoFranjasEstadoRealTime
        ...
        SELECT 
             UPPER(SUBSTRING(B.[FullName], CHARINDEX(',', B.[FullName]) + 2, LEN(B.[FullName])) + ' ' + LEFT(B.[FullName], CHARINDEX(',', B.[FullName]) - 1)) AS[TL]
            ,UPPER (B.[FullName ACCM]) AS[ACM]
            , B.[FullNameSupervisor]    AS [Director]
            ,B.[GeneralId]
            ,B.[FullName]
            ,B.[FullName ACCM]
            ,B.[FullNameSupervisor]
            ,B.[AM]
       FROM [TPCCP-DB141\SQL2016STD,5081].[AdHocReports].[dbo].[tbHeadCountFullView] AS B WITH(NOLOCK)
       WHERE B.[Client] = 'Mercado Libre'
    ...
    DROP TABLE #tmpConsolidadoFranjasEstadoRealTimeQA;
     ...
     INSERT INTO #tmpConsolidadoFranjasEstadoRealTimeQA
        ...
        /*======================================== Carga a la tabla fisica ======================================*/
            ...
            INSERT INTO  tbConsolidadoFranjasEstadoRealTime 
            SELECT                        
                 [fecha]                
                ,[inicio]               
                ,[fin]                  
